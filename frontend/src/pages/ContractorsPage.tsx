...
const ContractorsPage: React.FC = () => {
  ...
  const fetchContractors = async () => {
    try {
      ...
      // Используем правильный API для получения профилей исполнителей
      const profiles = await apiService.getAllContractorProfiles(100, 0);
  ...
  const handleUpdateContractor = async (data: ContractorProfileCreate) => {
    ...
    try {
      if (user?.role === UserRole.ADMIN) {
        ...
        await apiService.updateContractorProfileByAdmin(
          editingContractor.user_id,
          data,
        );
      } else {
  ...
  return (
    <Box>
      ...
      >
        <Typography variant='h4'>Исполнители</Typography>
        {canEdit() && (
          <Button
            variant='contained'
            startIcon={<Add />}
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
                <TextField
                  ...
                  {...register('last_name')}
                />
                <TextField fullWidth label='Имя' {...register('first_name')} />
                <TextField
                  fullWidth
                  label='Отчество'
                  {...register('patronymic')}
