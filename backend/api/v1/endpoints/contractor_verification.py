...
async def get_contractor_profile_extended(
    ...
    contractor_dict = {
        ...
        "passport_series": contractor.passport_series or "",
        "passport_number": contractor.passport_number or "",
        "passport_issued_by": contractor.passport_issued_by or "",
        "passport_issued_date": contractor.passport_issued_date or "",
        "passport_issued_code": contractor.passport_issued_code or "",
        "birth_date": contractor.birth_date or "",
        "birth_place": contractor.birth_place or "",
        "inn": contractor.inn,
        ...
        "security_verified_at": contractor.security_verified_at.isoformat() if contractor.security_verified_at and isinstance(contractor.security_verified_at, (date, datetime)) else (contractor.security_verified_at or None),
        "manager_verified_at": contractor.manager_verified_at.isoformat() if contractor.manager_verified_at and isinstance(contractor.manager_verified_at, (date, datetime)) else (contractor.manager_verified_at or None),
...
async def verify_contractor(
    ...
    try:
        if verification_data.verification_type == "security":
            ...
            if verification_data.approved:
                # Отправляем уведомление об одобрении менеджером
                await send_manager_approval_email(contractor_id, db, verification_data.notes)
            else:
                # Отправляем уведомление об отклонении менеджером
                await send_manager_rejection_email(contractor_id, db, verification_data.notes)
    except Exception as e:
    ...
    for verification in verifications:
        ...
        if contractor:
            ...
            verification_dict = {
                ...
                "contractor": {
                    "id": contractor.id,
                    "first_name": contractor.first_name if contractor.first_name is not None else (user.first_name if user and user.first_name else ""),
                    "last_name": contractor.last_name if contractor.last_name is not None else (user.last_name if user and user.last_name else ""),
                    "patronymic": contractor.patronymic if contractor.patronymic is not None else "",
                    "phone": contractor.phone if contractor.phone is not None else (user.phone if user and user.phone else ""),
                    "email": contractor.email if contractor.email is not None else (user.email if user and user.email else ""),
                    "telegram_username": contractor.telegram_username if contractor.telegram_username is not None else "",
                    "inn": contractor.inn if contractor.inn is not None else "",
                    "passport_series": contractor.passport_series if contractor.passport_series is not None else "",
                    "passport_number": contractor.passport_number if contractor.passport_number is not None else "",
                    "passport_issued_by": contractor.passport_issued_by if contractor.passport_issued_by is not None else "",
                    "passport_issued_date": contractor.passport_issued_date if contractor.passport_issued_date is not None else "",
                    "passport_issued_code": contractor.passport_issued_code if contractor.passport_issued_code is not None else "",
                    "birth_date": contractor.birth_date if contractor.birth_date is not None else "",
                    "birth_place": contractor.birth_place if contractor.birth_place is not None else "",
...
async def send_manager_approval_email(contractor_id: int, db: Session, notes: Optional[str] = None):
    """Отправляет email уведомление об одобрении менеджером"""
    try:
        from services.email_service import email_service
        ...
        subject = f"Профиль полностью одобрен - Добро пожаловать в команду!"
        message_html = f"""
        <html>
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
            <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
                <h2 style="color: #2e7d32;">Ваш профиль полностью одобрен!</h2>
                <p>Здравствуйте, {contractor_name}!</p>
                <p>Поздравляем! Ваш профиль прошел все проверки и получил окончательное одобрение менеджера.</p>
                <div style="background: #e8f5e9; padding: 15px; border-radius: 5px; margin: 20px 0; border-left: 4px solid #2e7d32;">
                    <p><strong>Теперь вы можете:</strong></p>
                    <ul>
                        <li>Просматривать все доступные заявки</li>
                        <li>Откликаться на заявки</li>
                        <li>Получать уведомления о новых заявках</li>
                        <li>Работать с заказчиками через платформу</li>
                    </ul>
                </div>
                <p style="margin-top: 30px;">
                    <a href="{dashboard_url}" style="background: #2e7d32; color: white; padding: 12px 24px; text-decoration: none; border-radius: 5px; display: inline-block;">
        ...
        logger.info(f"📧 Email об одобрении менеджером отправлен исполнителю {contractor_id} ({user.email})")
    except Exception as e:
        logger.error(f"❌ Ошибка отправки email об одобрении менеджером: {e}")

async def send_manager_rejection_email(contractor_id: int, db: Session, reason: Optional[str] = None):
    """Отправляет email уведомление об отклонении менеджером"""
    try:
        from services.email_service import email_service
        ...
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
            <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
                ...
                <p>Здравствуйте, {contractor_name}!</p>
                <p>Спасибо за интерес, проявленный к нашей платформе.</p>
                <p>К сожалению, после проверки менеджером мы не готовы продолжить работу с Вами на данный момент.</p>
                {f'<div style="background: #ffebee; padding: 15px; border-radius: 5px; margin: 20px 0; border-left: 4px solid #d32f2f;"><p><strong>Причина:</strong> {reason}</p></div>' if reason else ''}
                <p>Если у Вас возникнут вопросы, пожалуйста, свяжитесь с нашей службой поддержки.</p>
                <p style="color: #666; font-size: 12px; margin-top: 30px;">
                    С уважением,<br>
        ...
        logger.info(f"📧 Email об отклонении менеджером отправлен исполнителю {contractor_id} ({user.email})")
    except Exception as e:
        logger.error(f"❌ Ошибка отправки email об отклонении менеджером: {e}")
