...
def get_repair_requests(
    ...
    try:
        ...
        if current_user.role == "customer":
            ...
            query += " WHERE (r.assigned_contractor_id = $1 OR r.id IN (SELECT request_id FROM contractor_responses WHERE contractor_id = (SELECT id FROM contractor_profiles WHERE user_id = $2)))"
        ...
        if status_filter:
            if params:
                query += " AND r.status = $" + str(len(params) + 1)
            else:
                query += " WHERE r.status = $1"
            params.append(status_filter)
        
        # Сортировка и пагинация
        query += " ORDER BY r.created_at DESC LIMIT $" + str(len(params) + 1) + " OFFSET $" + str(len(params) + 2)
        params.extend([limit, skip])
        
        result = db.execute(text(query), tuple(params)).fetchall()
